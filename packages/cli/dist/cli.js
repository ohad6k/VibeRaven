  ...
  return {
    ...
    missionGraph: {
      areas: [
        ...
        { key: "database", label: "Database", readinessPercent: hasSupabase ? (0, import_node_fs.existsSync)((0, import_node_path.join)(workspacePath, "supabase", "migrations")) ? 100 : 50 : 100 }
      ]
    },
    stackWiring: { detected: [packageJson && "package.json", envExample && ".env.example", hasDeploy && "deployment-config", hasSupabase && "supabase"].filter(Boolean) },
...
async function writeArtifacts(workspacePath, artifact) {
  ...
  await (0, import_promises.writeFile)((0, import_node_path.join)(out, "context-map.json"), JSON.stringify({ version: 1, generatedAt: artifact.scannedAt, workspacePath, gateStatus: artifact.verificationSummary.status, openGapIds: artifact.gaps.map((gap) => gap.id), detectedEvidence: artifact.stackWiring.detected }, null, 2) + "\n");
...
function localState(cwd = process.cwd(), artifact) {
...
async function startUi(port, cwd) {
