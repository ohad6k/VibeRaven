...
2. Do NOT run verify after every file patch. Batch heals, then `npx -y viberaven@latest --verify` once per loop.
...
- Use `npx -y viberaven@latest next --json` or `npx -y viberaven@latest prompt --gap <id>` for one focused repo-code fix at a time.
- For Vercel + Supabase launch checks, run `npx -y viberaven@latest audit --vercel-supabase`.
- Run `npx -y viberaven@latest --verify` after one fix.
- Run `npx -y viberaven@latest --strict` before deploy or CI pass.
- Use `npx -y viberaven@latest --heal --plan --gap <id>` before any heal apply.
- Only use `npx -y viberaven@latest --heal --apply --gap <id> --yes` for supported low-risk repo-code recipes.
- Preview local rule installation with `npx -y viberaven@latest init --agents all --dry-run`.
- Cleanup is non-destructive: only run `npx -y viberaven@latest clean --plan` to create a reviewable cleanup plan.
